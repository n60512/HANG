...
def _train_HANN_model(data_preprocess):
    ...
    res, itemObj, userObj = data_preprocess.load_data(sqlfile=item_net_sql, testing=False, 
        table= opt.selectTable, rand_seed=opt.train_test_rand_seed)  # for clothing.

    # Generate voc & (User or Item) information , CANDIDATE could be USER or ITEM
    voc, ITEM, candiate2index = data_preprocess.generate_candidate_voc(res, 
    ...
    correspond_batches, correspond_asin_batches = data_preprocess.GenerateTrainingBatches(ITEM_CONSUMER, itemObj, voc, 
        net_type = correspond_model_net_type,
    ...
    rating_regresstion.hybird_train(opt.selectTable, isStoreModel=True, WriteTrainLoss=True, store_every = opt.save_model_freq, 
        use_pretrain_item=False, isCatItemVec=not True, pretrain_wordVec=pretrain_wordVec)
    ...
    if(opt.hybird == 'Y'):
        user_base_sql = R'HANG/SQL/cloth_candidate_asin.sql'
        res, itemObj, userObj = data_preprocess.load_data(sqlfile=user_base_sql, 
            testing=True, table= opt.selectTable, rand_seed=opt.train_test_rand_seed)  # for clothing.

        # Generate voc & (User or Item) information , CANDIDATE could be USER or ITEM
        USER, uid2index = data_preprocess.generate_candidate_voc(res, 
            having_interaction=opt.having_interactions, 
            net_type = 'user_base',
            generate_voc=False)
        ...
        for _item in CANDIDATE:
            ...
            newUSER.append(USER[user_index])   

        # Generate `training set batches`
        correspond_batches, correspond_asin_batches = data_preprocess.GenerateTrainingBatches(newUSER, itemObj, voc, 
            net_type = 'user_base', 
            num_of_reviews= 4, 
            batch_size=opt.batchsize,
            testing=True)
    ...
    if(opt.mode == "test" or opt.mode == "both"):
        ...
        for Epoch in range(0, opt.epoch, opt.save_model_freq):
            ...
            RMSE = rating_regresstion._hybird_evaluate(IntraGRU, InterGRU, correspond_IntraGRU, correspond_InterGRU, MFC,
                testing_batches, testing_asin_batches, testing_batch_labels, candidate_asins, candidate_reviewerIDs,
                correspond_batches, 
