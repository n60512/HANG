...
def Train(myVoc, table, training_batches, training_item_batches, candidate_items, candidate_users, training_batch_labels, 
    ...
    asin, reviewerID = pre_work._read_asin_reviewer(table='clothing_')
    ...
    InterGRU = HANN(hidden_size, embedding, asin_embedding, reviewerID_embedding,
            n_layers=1, dropout=dropout, latentK = latentK, isCatItemVec=isCatItemVec , netType=opt.netType, method=inter_method)
    ...
    for batch_ctr in range(len(training_batches[0])): #how many batches
        for idx in range(len(validate_batch_labels)):
            for reviews_ctr in range(len(training_batches)): #loop review 1 to 5
                ...
                if(isWriteAttn):
                    if(opt.netType=='user_base'):
                        current_candidates = current_reviewerIDs
                    elif(opt.netType=='item_base'):
...
if __name__ == "__main__":
    ...
    pre_work = Preprocess(setence_max_len=opt.setence_max_len, use_nltk_stopword=opt.use_nltk_stopword)
    
    res, itemObj, userObj = pre_work.load_data(sqlfile=opt.sqlfile, testing=False, table= opt.selectTable, rand_seed=opt.train_test_rand_seed)  # for clothing.

    # Generate voc & (User or Item) information , CANDIDATE could be USER or ITEM
    voc, CANDIDATE = pre_work.generate_candidate_voc(res, 
        having_interaction=opt.having_interactions, 
        netType = opt.netType)

    # pre-train words
    if(opt.use_pretrain_word == 'Y'):
        # pretrain_wordVec = load_PretainWord(voc, pretrain_words)
        weights_matrix = pre_work.load_pretain_word(voc, pretrain_words)
    ...
    if(opt.mode == "train" or opt.mode == "both"):
        ...
        training_batch_labels, candidate_asins, candidate_reviewerIDs = pre_work.get_train_set(CANDIDATE, 
            itemObj, 
            userObj, 
            batchsize=40, 
            num_of_reviews=5, 
        ...
        training_batches, training_asin_batches = pre_work.GenerateTrainingBatches(CANDIDATE, candidateObj, voc, 
            netType = opt.netType, 
    ...
    if(opt.mode == "test" or opt.mode == "showAttn" or opt.mode == "both"):
        ...
        res, itemObj, userObj = pre_work.load_data(sqlfile=opt.sqlfile, testing=True, table=opt.selectTable, rand_seed=opt.train_test_rand_seed)   # clothing
        # If mode = test, won't generate a new voc.
        CANDIDATE = pre_work.generate_candidate_voc(res, having_interaction=opt.having_interactions, generate_voc=False, 
            netType = opt.netType)

        testing_batch_labels, candidate_asins, candidate_reviewerIDs = pre_work.get_train_set(CANDIDATE, 
            itemObj, 
            userObj, 
            batchsize=40, 
            num_of_reviews=5, 
        ...
        # Generate testing batches
        testing_batches, testing_asin_batches = pre_work.GenerateTrainingBatches(CANDIDATE, userObj, voc,  netType = opt.netType,
